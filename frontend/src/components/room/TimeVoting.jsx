...
export default function TimeVoting({ roomId, event, darkMode, isOwner, canEdit }) {
  ...
  const leadingOption = options.length > 0 
    ? options.reduce((max, opt) => (opt.votes_count || 0) > (max.votes_count || 0) ? opt : max, options[0])
    : null
      ...
      {options.length === 0 ? (
        ...
        <div className="space-y-2">
          {options.map(option => {
            ...
            const votePercentage = leadingOption && (leadingOption.votes_count || 0) > 0
              ? Math.round(((option.votes_count || 0) / leadingOption.votes_count) * 100)
              : 0
            ...
            return (
              ...
              >
                {leadingOption && (leadingOption.votes_count || 0) > 0 && (
                  <div 
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                        {option.creator_name || 'Участник'} • {option.votes_count || 0} голосов
                      </p>
                      {isLeading && (option.votes_count || 0) > 0 && (
