    ...
    ws.on('message', async (data) => {
      try {
        ...
        if (message.type === 'message') {
          const { content, reply_to_id, attachments } = message;
          
          if ((!content || content.trim().length === 0) && (!attachments || attachments.length === 0)) return;
          ...
          try {
              ...
              for (const att of attachments) {
                await connection.execute(
                  `INSERT INTO message_attachments (message_id, file_name, file_url, file_type, file_size) 
                   VALUES (?, ?, ?, ?, ?)`,
                  [messageId, att.name, att.url, att.type, att.size]
                );
              }
            }
            ...
            // Получаем вложения
            const [savedAttachments] = await connection.execute(
              'SELECT * FROM message_attachments WHERE message_id = ?',
              [messageId]
            );
        ...
        if (message.type === 'delete_message') {
          ...
          try {
            ...
            if (messages.length > 0) {
              ...
              if (isOwner || isAuthor) {
                // Вложения удалятся автоматически благодаря FOREIGN KEY ON DELETE CASCADE
